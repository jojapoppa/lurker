...
use crate::ser::{Readable, Reader, Writeable, Writer, ProtocolVersion};
use crate::genesis;
use crate::global;
use chrono::prelude::{DateTime, Utc};
use randomx_rs::{RandomXCache, RandomXFlag, RandomXVM, RandomXError};
...
pub fn pow_size(bh: &mut BlockHeader, diff: Difficulty) -> Result<(), crate::core::block::Error> {
    ...
    let mut vm = match RandomXVM::new(RandomXFlag::FLAG_DEFAULT, Some(pow.cache.clone().unwrap()), None) {
        Ok(vm) => vm,
        Err(_) => {
    ...
    loop {
        ...
        if nonce == start_nonce {
            ...
            vm = match RandomXVM::new(RandomXFlag::FLAG_DEFAULT, Some(pow.cache.clone().unwrap()), None) {
                Ok(vm) => vm,
                Err(_) => {
                    let error = crate::core::block::Error::Pow(PowError::VMCreate);
                    return Err(error);
