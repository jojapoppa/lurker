...
use crate::grin::seed;
use crate::grin::sync;
use crate::p2p::{Capabilities, Seeding, Server as P2PServer};
use crate::pool::{self, PoolToChainAdapter, PoolToNetAdapterAlt};
...
pub type ServerTxPool = Arc<RwLock<pool::TransactionPool<PoolToNetAdapterAlt, PoolToChainAdapter>>>;
...
impl Server {
	...
	pub fn new(
		...
		let pool_net_adapter = Arc::new(PoolToNetAdapterAlt::new());
		let tx_pool = Arc::new(RwLock::new(pool::TransactionPool::new(
			config.pool_config.clone(),
			pool_adapter.clone(),
			pool_net_adapter.clone(),
		)));
		pool_net_adapter.set_tx_pool(pool::DandelionTxPool(Arc::new(tx_pool.clone())));
		...
		let dandelion_thread = dandelion_monitor::monitor_transactions(
			...
			pool::DandelionTxPool(Arc::new(tx_pool.clone())),
			pool_net_adapter,
			stop_state.clone(),
		)?;
