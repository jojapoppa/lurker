...
pub struct BlockHeaderPrintable {
	...
	pub nonce: u64,
	/// Size of the cuckoo graph (0 for RandomX)
	pub edge_bits: u8,
	/// Nonces of the cuckoo solution (single nonce as vec for RandomX compat)
...
impl BlockHeaderPrintable {
	pub fn from_header(header: &core::BlockHeader) -> BlockHeaderPrintable {
		// For RandomXProofOfWork: direct access to fields (no 'proof' sub-struct)
		let pow = &header.pow;
		BlockHeaderPrintable {
			...
			nonce: pow.nonce,
			edge_bits: pow.edge_bits(),
			cuckoo_solution: vec![pow.nonce], // Single nonce as Vec<u64> for API backward compat
			total_difficulty: pow.total_difficulty.to_num(),
			secondary_scaling: pow.secondary_scaling,
